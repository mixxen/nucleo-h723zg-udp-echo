# Rust UDP Echo Server for NUCLEO-H723ZG
...
- listens on UDP port 7 and echoes to the sender's actual source port; and
...
LAN8742A PHY <---RMII---> STM32 Ethernet driver
                    ...
                    shared embassy-net Stack handle
                         /                    \
                        v                      v
             Link/DHCP supervisor       UDP echo task
             LEDs + static fallback     receive -> send
```
...
3. `embassy_net::new` creates a network `Stack` and a `Runner`. The stack is
   ...
   processing.
4. Embassy's executor starts three tasks:
   - the network runner, which drives Ethernet, ARP, IPv4, ICMP, DHCP, and UDP;
   - the supervisor, which watches link/DHCP state and controls the LEDs; and
   - the UDP server, which waits for datagrams on port 7 and echoes them.
...
On 2026-07-23 this release configuration was built, programmed, and verified
on the connected NUCLEO-H723ZG. The board obtained `192.168.68.57`, answered
ping, and returned a byte-identical UDP payload from port 7.
